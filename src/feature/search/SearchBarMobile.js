...
const SearchBarMobile = ({ handleNavigate }) => {
  ...
  useEffect(() => {
    if (!suggestions.search.date) {
      dispatch(addDate(today.startOf("day").unix()));
    }
  ...
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    const travelEpoch = getTravelDateEpoch(
      suggestions.search.date || today
    );
    handleNavigate({
